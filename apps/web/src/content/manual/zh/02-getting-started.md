...
密码登录页有一个「保持登录状态」勾选框，**默认不勾选**（仅对密码登录生效；企业 SSO 登录固定为 7 天）：

- **不勾选**：登录状态在**关闭浏览器后即失效**，服务端也最多只认 12 小时。在公用电脑、会议室机器上请保持不勾选。
- **勾选**：登录状态保留 7 天（由部署的 `AUTH_SESSION_TTL_DAYS` 决定）。

两种情况下，只要你在持续使用，登录状态都会自动续期——不会出现「做到一半被踢下线」。反过来，长时间不用就会按上面的期限自然过期。想立刻让所有设备上的登录态失效，用「退出登录」或修改密码即可。

> [!TIP]
> a2wave 不提供「保存密码」功能，也不会把密码存在浏览器里。需要免输密码请用浏览器自带的密码管理器，或改用企业 SSO 登录。
