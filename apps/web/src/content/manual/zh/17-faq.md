...
最常见是 **飞书长连接被占用**：同一飞书 App ID 在单个 API 进程内只允许一条活跃长连接，先启动者优先。多个 Agent 接飞书请各用 **独立飞书应用**。连接状态可直接在「渠道」页的渠道卡片上查看（飞书 / Slack / Discord 都会显示各自的协议与实时状态），也可在 Agent「综合诊断」中查看。详见 [触发方式](/wiki/triggers)。
...
新建的标准 Docker 部署默认限制整个应用容器最多使用 2 核 CPU、3 GiB 内存、3.5 GiB 内存与 swap 合计，以及 512 个进程或线程。这些预算由所有 Agent 和工具共同使用，不是每个 Agent 各有一份；达到上限时，任务可能失败。

请管理员先检查主机资源和 [运行记录](/wiki/runs)，再决定降低并发或调整部署预算，不要直接反复重试重型任务。多个同机服务的预算总和应给操作系统留出余量。

已有部署不会仅因拉取新镜像而自动获得限额：管理员需要核对并更新容器配置，再在任务结束后重新创建容器。原生或 PM2 部署也需要由管理员在系统服务层配置对应限额。
