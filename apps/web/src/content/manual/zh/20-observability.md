...
默认情况下，导出的只有**元数据**：耗时、Token、模型、状态、工具名。用户的提问、Agent 的回复、工具入参与输出、报错文本都**不会**离开 a2wave。

> [!WARNING]
> 采集内容会把 Agent 读到的东西一起带出去：工具输出里可能有配置文件、代码、接口返回。a2wave 会掩码它注入的凭据，以及常见格式的密钥（私钥块、`Bearer`、GitLab / GitHub / AWS / Slack 令牌、JWT、`password=…` 这类赋值的值），但**没有固定格式的密钥和敏感业务数据无法识别**。只把采集端指向你信任、且访问范围与留存策略受控的平台。

打开「采集内容」后，这些内容会一并写进 Trace，便于排查具体某次执行说了什么、调了什么、工具返回了什么。工具输出只用于导出：它不会写进 a2wave 的运行日志，也不会出现在界面的执行过程里。目前 Claude Code、Codex、Qoder、Trae 会上报工具输出，其他 Provider 的工具 span 没有输出。
