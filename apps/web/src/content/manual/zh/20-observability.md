...
1. 进入「设置 → 可观测性」。
2. 填写**采集端地址**。填基础地址即可，例如 `http://otel-collector:4318`，系统会自动补上 `/v1/traces`；如果你的平台给的是完整上报地址，也可以直接填。支持内网地址和本机地址。
3. 如果采集端需要鉴权，点「添加请求头」，填入名称（如 `Authorization`）和值。
4. 点「保存」，再点「测试连接」。看到「测试 span 已送达采集端」就说明链路通了。
5. 打开「启用导出」并保存。此后的每次执行都会导出。
...
「设置 → 可观测性」底部会显示最近一次成功上报的时间、最近的错误，以及被丢弃的 span 数量。

| 现象 | 可能原因 |
|------|----------|
| 测试连接提示「上报失败」 | 地址或端口不对、采集端没有开启 OTLP/HTTP 接收、网络不通 |
