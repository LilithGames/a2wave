...
> [!NOTE]
> 部分 Provider 对服务器上安装的 CLI 有**最低版本要求**（Qoder CLI ≥ 1.0.0、Trae CLI ≥ 0.120.0、OpenCode CLI ≥ 1.18.0、Kimi Code CLI ≥ 0.30.0、Pi CLI ≥ 0.84.3）。这是**下限**不是精确版本，更高版本同样可用。检测服务器登录态时会同时上报已安装版本，低于要求会给出升级提示；绑定该 Provider 的 Agent 跑「综合诊断」也会报出一条错误级检查，写明已装版本与最低要求。版本过低时模型拉取、登录检测等功能可能不可用，但平台不会因此阻断运行。平台锁定的版本都满足这些要求。
...
在 Agent 配置里展开一个使用 localSession 的 Provider 时，界面会**自动检测一次**服务器上的登录态，并给出「已登录 / 登录态已失效 / 未登录 / CLI 未安装 / 检测失败」以及已安装的 CLI 版本；在服务器上补登录之后，点旁边的「重新检测」即可当场确认。整个部署共用一套登录态，它一旦过期，所有依赖它的 Agent 会同时开始失败——运行记录里成片的失败通常先到这里检查。

> [!IMPORTANT]
> 「已登录」后面还会标注**「已验证」还是「仅本地凭证」**，这个区别很关键：多数 CLI 的登录检测只确认服务器上存在凭证文件，**不会**去问服务商这份凭证是否仍被接受——token 被吊销或过期后，它照样显示已登录，而每次运行都会 401。目前 Codex 会额外执行 `codex doctor` 真正向服务商确认，因此能给出「已验证」；其余 Provider 显示「仅本地凭证」，真正的验证发生在运行时。
>
> 正因如此，Provider 区域还会显示该 Agent **最近一次运行失败的错误**。若错误形如 401 / 403 / token 被吊销，界面会直接提示「可能是登录态失效」——一次真实运行的报错，比任何本地探测都可靠。
