...
飞书、Slack、Discord、Telegram、QQ 机器人以及 A2A 均支持。定时触发与 GitLab / GitHub 触发没有可回复的会话，因此不支持。
...
`/new` 立即开启新会话，丢弃上一轮的上下文。**所有聊天渠道**均支持——飞书、Slack、Discord、Telegram、QQ 机器人——且**仅在单聊生效**：群聊中的 `/new` 会作为普通文本交给 Agent，因为群聊本身就有重新开始的方式（另起一条顶层消息，或开一个新话题）。
