...
## 用 `/status` 自助查询 Agent 状态

向机器人发送 **`/status`**（或 `/状态`），会立即返回该 Agent 的当前情况。这条回复由平台直接生成，**不经过模型**，不产生 Run，也不占用队列——所以 Agent 正忙时你照样问得到，Agent 完全跑不起来时你也拿得到答案。

回复包含三部分：

| 部分 | 内容 |
|------|------|
| **元信息** | 名称、启用与发布状态、当前使用的模型、已绑定的渠道 |
| **队列** | 空闲 / 繁忙 / 队列已满，以及正在执行数与并发上限的对比、排队数量 |
| **健康** | 当前是否可执行；不可执行时逐条列出阻塞原因（未绑定 Provider、CLI 未安装、版本低于最低要求、凭据缺失等） |

飞书、Slack、Discord、Telegram、QQ 机器人、A2A，以及**调试面板与对话页**均支持——排查问题时可以直接在同一个面板里查状态。定时触发与 GitLab / GitHub 触发没有可回复的会话，因此不支持。
...
# CLI —— Agent 不可执行时退出码为 1，可直接用作 CI 卡点
a2wave agents status <agent> --json
```

> [!TIP]
> 回复语言由 **Agent 详情 → 其它设置 → 命令响应使用语言** 决定。保持「自动」时，你用哪种语言提问就用哪种语言回答。

### 另一条命令：`/new`

`/new` 立即开启新会话，丢弃上一轮的上下文。**所有聊天渠道**均支持——飞书、Slack、Discord、Telegram、QQ 机器人，以及调试面板与对话页——且**群聊中不生效**：群聊中的 `/new` 会作为普通文本交给 Agent，因为群聊本身就有重新开始的方式（另起一条顶层消息，或开一个新话题）。

单独发送即重置会话，发送 `/new <内容>` 则在新会话中直接处理 `<内容>`。
...
同一条会话线里的连续提问，Agent 能看到之前几轮的上下文。会话线怎么划分、什么时候自动重开，取决于聊天场景：

| 场景 | 会话线怎么分 | 什么时候开新会话 |
|------|--------------|------------------|
| 单聊（私信） | 整个私聊就是**一条**会话线 | 距上一轮结束超过 **2 小时**；或发送 `/new` 立即重开 |
| 群聊里 @ 机器人 | 每条被 @ 的消息各起一条线 | 每次新的 @ 都是新会话 |
| 群聊里回复机器人 | 一条回复链是一条线 | 另起一条回复链；同一条链不会因为间隔太久而断 |
| 话题群 | 一个话题是一条线 | 新开一个话题；同一话题内不会因为间隔太久而断 |

> [!NOTE]
> 私聊里**引用回复**一条旧消息，并不会把那一轮的上下文接回来——私聊只有一条会话线，引用哪条都一样，超过 2 小时照样开新会话。想接着旧上下文说，就在 2 小时内继续；想立刻从零开始，发 `/new`（仅私聊有效，群聊和话题里另起消息/话题即可）。
...
- **正文保留**：Agent 的说明正文和问答说明会一起显示在卡片中；提交或取消后，原卡片仍保留这些内容，并显示操作结果。
...
在 Agent 的「发布」页打开「QQ 官方机器人」卡片，选择以下任一方式准备凭证：

1. **扫码创建（推荐）**：点击「扫码创建」，用手机 QQ 扫描二维码并在腾讯页面确认。完成后，App ID 与 App Secret 会自动回填到表单；再点击「保存配置」，打开卡片开关并发布。
2. **已有机器人**：直接填写腾讯 QQ 开放平台已有机器人的 App ID 与 App Secret。

首个版本支持 QQ 群 @ 消息和 QQ 单聊。群聊仅在 @ 机器人时触发，普通群消息不会触发；群聊中的 `/new` 作为普通文本交给 Agent，不会重置会话。QQ 频道及频道私信暂不支持。

QQ 群聊按“机器人 + 群”共享会话，不同群成员的消息会进入同一上下文。每条消息都会同时向 Agent 提供发送者的 `member_openid`，以及 QQ Gateway 可用时的 `username`，便于区分当前发言人。距离上一条用户消息不超过 8 小时会继续该群会话；超过 8 小时后，下一条消息自动开启新会话。

QQ 单聊按“机器人 + 用户”维护会话。距离上一条用户消息不超过 2 小时会继续同一会话；超过 2 小时后，下一条消息自动开启新会话。发送 `/new` 可立即开启新会话，发送 `/new <内容>` 则会在新会话中直接处理 `<内容>`。

回复可以引用原消息、发送新消息或关闭。QQ 对被动回复有较短时效；若 Agent 执行结束时原消息回复锚点已经失效，a2wave 会按 AstrBot 的行为降级为发送新消息。群聊和 QQ 单聊支持把 Agent 产物先上传到腾讯媒体接口再发送。用户发来的官方附件会按平台统一的文件类型、数量和大小限制下载后交给 Agent。

> [!WARNING]
> 同一个 QQ App ID 在单个 API 进程内只允许一个 Agent 持有官方 Gateway 连接。多个 Agent 请创建不同的 QQ 官方机器人。Agent 的受信任归属人和编辑者可以查看 App Secret 以维护渠道；扫码任务的绑定密钥与完成后的明文 Secret 不写入审计详情。
