...
> [!NOTE]
> 「直连端点」无法发现远端是否支持流式调用，因此 A2A 1.0 与 0.3 都会有意使用阻塞式 `SendMessage`，不能实时显示远程子 Agent 输出。如需流式输出，并且对方声明支持，请改用「Agent Card 发现」。
...
> [!NOTE]
> **配额是按 CLI 登录态计的，不是按 Agent。** 同一台服务器上所有 Agent 共享同一个 `glab` / `gh` 登录，因此 forge 的 API 限流是**每 host 共享预算**。按默认值估算：20 个仓库 × 每 30 秒 1 次 ≈ 40 请求/分钟，而 GitLab 认证用户默认额度约 2000 请求/分钟，单个 Agent 余量充足；但配置多个高频 Agent 时需要按总量估算。

### 配置项

| 配置 | 说明 |
|------|------|
| **仓库** | 直接粘贴仓库地址，例如 `https://gitlab.example.com/group/subgroup/repo`。Host 与项目路径由地址自动拆分并显示在输入框下方，无需分两个字段填写；MR/PR 页面链接会自动还原成所属仓库。最多 5 个。仓库是**逐个串行**拉取的（不并发），这样「读取」和「处理」之间没有时间窗，避免配置变更或停用时仍按旧配置触发；代价是仓库数上限较低——需要监控更多仓库请拆成多个 Agent。 |
