...
打开 Agent 的「发布」标签，可以看到所有渠道以卡片平铺展示，一眼就能看出哪些已经启用。顶部的筛选可以按类别收窄：全部 / API / 协议 / 聊天机器人 / 定时 / 网页应用 / Git 仓库触发。
...
改完配置后，点页面底部的「发布 / 更新发布」让 Agent 正式上线；已上线的 Agent 在弹窗内保存配置会即时生效，且只影响该渠道，不会打断其它渠道的连接。
...
| **待发布生效** | 开关刚被改动但尚未发布。连接状态**不会**随开关立即改变，点击「发布 / 更新发布」后才会真正建立或断开。 |
...
配置完成后，在 a2wave 的「发布」页点「飞书机器人」卡片的「配置」，填写 **App ID / App Secret** 及触发与回复策略：
...
> [!WARNING]
> **一 App 一单连接**。同一飞书 App ID 在单个 API 进程内只允许 **一条** 活跃 WebSocket，**先占先得、后启动不得抢占**。多个 Agent 要接飞书，必须各用 **独立的飞书应用**。连接状态可直接在「发布」页的飞书卡片上查看（被抢占时显示「本实例无连接」），也可在 Agent「综合诊断」中查看。
...
完成 Slack 后台设置后，在「发布」页的 Slack 卡片点「配置」，填写 **App ID**（形如 `A...`）、**App Token**（`xapp-...`）和 **Bot User OAuth Token**（`xoxb-...`）。频道默认只在 @Bot 时触发；私信始终触发。测试频道消息前，先将 Bot 加入频道，再发送 `@Bot hello`；也可以直接私信 Bot。
...
在「发布」页的 Discord 卡片点「配置」，填写 **Application ID** 与 **Bot Token**。在 Discord Developer Portal 中开启 **Message Content Intent**，邀请 Bot 时授予 `View Channels`、`Send Messages`、`Read Message History` 与 `Attach Files` 权限；若要在线程频道回复，还需 `Send Messages in Threads`。
...
在「发布」页的「对话网页」卡片点「配置」，保存后打开卡片上的开关即可得到链接：
...
在 Agent 的「发布」页启用「A2A 协议」，复制页面给出的 Agent Card URL 与调用端点。调用方先读取 Agent Card，再按 Card 声明的协议版本和端点发起请求。
...
让 Agent 按 Cron 在指定时间自动创建并执行 Run（如每日代码审查、周报、巡检）。在「发布」页的「定时触发」卡片点「配置」：
...
> [!NOTE]
> **配额是按 CLI 登录态计的，不是按 Agent。** 同一台服务器上所有 Agent 共享同一个 `glab` / `gh` 登录，因此 forge 的 API 限流是**每 host 共享预算**。按默认值估算：5 行 × 每 30 秒 1 次 ≈ 10 请求/分钟（群组范围翻页时最多 5 倍，即 50 请求/分钟），而 GitLab 认证用户默认额度约 1200–2000 请求/分钟，单个 Agent 余量充足；但配置多个高频 Agent 时需要按总量估算。

### 配置项

| 配置 | 说明 |
|------|------|
| **监听范围**（仅 GitLab） | 每一行可以选「单个仓库」「整个群组」或「全部可见」，详见下方[监听范围](#监听范围仅-gitlab)。GitHub 只支持单个仓库。 |
...
| **全部可见** | 不用填 | 当前 `glab` 登录凭据能看到的所有仓库，适合个人助理型 Agent |
...
> [!WARNING]
> **范围越大，越要配合过滤条件。** 「全部可见」会监听你能看到的每一个仓库，在大型 GitLab 上这可能是几百个开启中的 MR。建议同时设置「目标分支过滤」，并谨慎勾选「有新评论」，否则 Agent 会被大量与你无关的活动唤醒。
>
> 另外，群组范围下**单轮最多翻 5 页（500 个开启中的 MR）**。超出后本轮不再推断「已合并或关闭」（原因同下方的单页限制），其余事件正常触发；日志里会提示你缩小范围。
