...
| 访问范围 | 说明 |
|----------|------|
| 全体企业用户 | 任何持有企业 OIDC 签发、`aud` 在白名单内，且能从 JWT 或标准 UserInfo 接口取得邮箱的员工都能调用。 |
| 指定企业用户 | 只有名单内的邮箱能调用，其余一律拒绝。名单可以搜索同事添加，也可以直接输入邮箱。 |

如果验签后的 JWT 没有邮箱声明，a2wave 会把同一个 bearer token 发给 OIDC Discovery 中的标准 `userinfo_endpoint`，并要求返回的 `sub` 与 JWT 一致。JWT 已带邮箱时直接使用，不会请求 UserInfo。两个来源都没有邮箱时才拒绝调用。该回退要求调用方提供能被 UserInfo 接受的 access token；不带邮箱的 ID token 不够用。

选择 **指定企业用户** 时，名单为空表示**没有人**能调用（拒绝而不是放行），所以发布前至少要添加一个成员。名单按邮箱比对从 OIDC JWT 或 UserInfo 响应取得的已验证邮箱，大小写不敏感。
...
| 新接入方一直 401 | 多半是调用方拿到了面向错误资源的 token，或传入了不带邮箱且 UserInfo 不接受的 ID token。让调用方申请一枚面向已配置 a2wave 受众的 access token；不要把观察到的任意 `aud` 照抄进白名单，因为该 token 可能原本是为其它服务签发的 |
| 某段时间所有调用方一起 503「身份提供方不可用」 | a2wave 连不上 IdP（Discovery / JWKS / UserInfo 失败），不是调用方凭据的问题，无需重新签发 token；排查出网、DNS、代理即可 |
