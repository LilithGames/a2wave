...
export async function createRecordedA2AExecuteFn(c: Context, agent: AgentRow): Promise<ExecuteFn> {
  ...
  // An upstream Agent's agent-router forwards the traceparent of its attempt span, so this run
  // joins the caller's trace. Optional-called for the same reason as `c.get` below.
  ...
  return async (taskId, payload, options) => {
    ...
    try {
      await db.insert(runs).values({
        ...
        ...(traceParent ? { executionMetadata: { traceParent } } : {}),
      })
    } catch (err) {
