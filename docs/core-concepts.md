...
1. Sets the Agent's working directory to a **per-Agent worktree** (`<workspacesPath>/agent-<agentId suffix>`, persistent, advanced to the synced HEAD on every run while its tracked files are clean). Agents sharing one Source therefore never share a working directory — a run of one Agent re-mounting skills/config cannot delete the files another Agent's in-flight run is executing. If the worktree cannot be created, the run degrades to the Source's shared `localPath`.
