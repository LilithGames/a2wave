...
An Agent's `providerChain` stores its per-binding model, authentication mode, and credentials, plus the two model-adjacent execution controls: `reasoningEffort` and `fastMode`. They live on the binding rather than on the Agent because a chain mixes Providers and models for fallback, and the legal effort levels follow the model — one Agent-wide value would be invalid for at least one entry of any mixed chain. Both are re-applied with the binding, so a provider fallback swaps them along with the model instead of handing the next CLI a level it rejects.

The **levels are never hard-coded**: the manifest declares only whether a CLI accepts a reasoning setting (`reasoningEffort`) and a fast switch (`fastMode`), while the legal values are reported per model by discovery alongside the model ids (`GET /api/providers/probe-models` → `modelCapabilities`). They genuinely differ — codex advertises `ultra`, Claude never does; Claude Opus 4.5 has neither `xhigh` nor `max`; Haiku 4.5 accepts no effort at all. An absent entry means "not discovered" (a proxy standing in for the vendor endpoint returns bare model ids) and an empty list means "discovered, and there are none"; the UI keeps the two apart and passes nothing at all when a control is unset, so the CLI's own default is the only fallback. Nothing is validated at execution time — the model is only a string by then, and can still change mid-Run through fallback. `GET /api/agents/:id/diagnose` therefore reports a control bound to a Provider that has no such setting (`provider_reasoning_effort_unsupported` / `provider_fast_mode_unsupported`, severity `warn`); a level the *model* rejects needs a live probe to detect and is instead reported by the CLI itself, with the accepted set named in its error. Claude Code bindings in `apiKey` mode may also store `authHeaderStyle` (`x-api-key` or `bearer`); a missing value is resolved as `x-api-key` so existing and imported Agents retain their prior runtime contract. The same resolved style drives model probing and subprocess environment injection, without inspecting credential prefixes.

`localSession` always means a deployment-level shared CLI identity. In Docker, the API process has the explicit home `/home/appuser`; Compose persists that entire directory in `a2wave-cli-home`, so Codex, Cursor, OpenCode, Qoder, Trae, Kimi Code, and Pi configuration survives container replacement. Claude Code keeps its existing nested host `~/.claude` read-only mount for compatibility. Per-Agent runtime HOME isolation remains enabled for injected credential modes and is deliberately omitted for localSession; adapters may still place session history in an explicit per-Agent directory.

- **Claude Code** takes its reasoning level as `--effort <level>` (available since CLI 2.1.47, which is why that is the Provider's `minVersion` floor) and fast mode through an inline `--settings '{"fastMode":true}'` — the CLI exposes no flag for it. Fast mode additionally requires first-party Anthropic auth, a model that supports it, and an eligible plan; when any of those is missing the run silently proceeds at normal speed and reports `fast_mode_state` itself, which is why a2wave records the outcome rather than predicting it.
- **Codex CLI** uses `codex exec --json`, discovers workspace skills under `.codex/skills`, injects Agent-scoped MCP servers with `-c mcp_servers=...` per run without mutating `~/.codex/config.toml`, and authenticates through `OPENAI_API_KEY` or a local `codex login` session. Reasoning level and fast mode ride the same override channel (`-c model_reasoning_effort=...`, `-c service_tier="priority"`), re-passed on resume because `-c` — unlike `--sandbox` — is accepted there; codex validates the tier against what the selected model advertises and omits it with a warning rather than failing the run.
...
- **Execution**: Skills are synced as files to the agent's working directory (e.g., `.claude/skills/`) based on the Provider's `skillsDir` configuration. They are natively discovered and used by the execution engine (like Claude Code).
