...
- **Git SCM Agents run in per-Agent worktrees** (`agent-<agentId suffix>`, persistent, followSource). The shared `localPath` checkout is only synced into and no longer executed in — Agents sharing one Source stopped sharing a working directory because a run of Agent B re-mounting `.claude/skills` deleted the files Agent A's in-flight run was executing. The worktree sits on a branch named after it (so `git branch --show-current` answers — the contract git-sync keeps for the shared checkout; `A2WAVE_WORKSPACE_BRANCH` names it, while `GIT_BRANCH` keeps meaning the Source's tracked branch). On reuse it advances to the synced HEAD **only through the ancestor guard** (`merge-base --is-ancestor`): tracked modifications or unmerged agent commits pin it — with a warn log — and it unpins by itself once those commits reach the source branch. Same-Agent runs share their worktree **without an occupancy check** (serializing them would stall concurrent chat), but every channel records `runs.workDir` so the workspace-delete route's occupancy check sees in-flight runs. Agent deletion reclaims the worktree (branch included) via `scm.removeWorkspace`. CodeGraph stays reachable through an idempotent `.codegraph` symlink into the shared checkout. P4 keeps the shared checkout (no isolation possible). Worktree creation failure degrades to `localPath`, never fails the run.
